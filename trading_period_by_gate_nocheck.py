# -*- coding: utf-8 -*-
"""
Created on Wed May 13 11:57:27 2020
...
def pairs(pair, formate_time, table, min_data, tick_data, up_open_time, down_open_time, stop_loss_time, day1, maxi,
    ...
    use_adf = False
    # 波動太小的配對不開倉
    if (up_open_time + down_open_time) *table.Estd[pair] < cost_gate:
        trading_profit = 0
        trade = 0
        local_profit = 0
        local_open_num = 0
    ...
    for i in range(1, len(spread) - 2):
        ...
        if position == 0 and i != len(spread) - 3 and i <= 40:  # 之前無開倉
            if (spread[i] - up_open) * (spread[i + 1] - up_open) < 0 and spread[i + 1] < (
                    close + stop_loss):  # 碰到上開倉門檻且小於上停損門檻
                # 資金權重轉股票張數，並整數化
                w1, w2 = num_weight(table.w1[pair], table.w2[pair], tick_data[str(table.stock1[pair])][(i + 1)],
                                    tick_data[str(table.stock2[pair])][(i + 1)], maxi, capital)
                ...
                if use_adf and adfuller(spread1, regression='c')[1] > 0.05:  # spread平穩才開倉
                    ...
                    trade = trade + 1
            elif (spread[i] - down_open) * (spread[i + 1] - down_open) < 0 and spread[i + 1] > (
            ...
            '''
            if (spread[i] - down_open) * (spread[i + 1] - down_open) < 0:  # 空倉碰到下開倉門檻即平倉
                position = 666  # 平倉
                stock1_payoff = -w1 * slip(tick_data[str(table.stock1[pair])][(i + 1)], -table.w1[pair])
                stock2_payoff = -w2 * slip(tick_data[str(table.stock2[pair])][(i + 1)], -table.w2[pair])
                stock1_payoff, stock2_payoff = tax(stock1_payoff, stock2_payoff, position, tax_cost)  # 計算交易成本
    ...
    if position == -2 or position == -3 or position == -4 or position == 666:
        if dump:
            plot_spread(stock_date, table.stock1[pair], table.stock2[pair], spread, up_open, down_open, stop_loss,
                        close, local_profit, pos, position, up_open_time, down_open_time, stop_loss_time, folder_date)
    return   local_profit, local_open_num, trade_capital, trading
...
def plot_spread(stock_date, stock1, stock2, spread, up_open, down_open, stop_loss, close, local_profit, pos, position,
                up_open_time, down_open_time, stop_loss_time, folder_date):
    ...
    for x in range(1, len(pos)):
        if pos[x] != pos[x - 1]:
            plt.scatter(x, spread[x], color='', edgecolors='r', marker='o')
    plt.title(stock_date + ' s1:' + stock1 + ' s2:' + stock2 + ' 上開倉門檻:' + str(up_open_time) + ' 下開倉門檻:'
              + str(down_open_time) + ' 停損門檻:' + str(stop_loss_time))
    if position == 666:
        plt.xlabel('profit:' + str(local_profit) + ' 正常獲利平倉')
    elif position == -2:
        plt.xlabel('profit:' + str(local_profit) + ' 碰到停損門檻平倉')
    elif position == -3:
        plt.xlabel('profit:' + str(local_profit) + ' 結構性斷裂平倉(fore_lag5)')
    elif position == -4:
        plt.xlabel('profit:' + str(local_profit) + ' 時間結束，強迫平倉')
