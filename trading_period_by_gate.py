...
def pairs( pair , formate_time , table , min_data , tick_data , open_time , close_time , stop_loss_time , day1 , maxi , tax_cost , cost_gate , capital ):
    trade_capital = 0
    cpA,cpB = 0,0
    ...
    if (open_time + close_time) * table.stdev[pair] < cost_gate: 
        ...
        return  local_profit , local_open_num, trade_capital 
    ...
    spread = table.w1[pair] * np.log(tick_data[ str(table.stock1[pair]) ]) + table.w2[pair] * np.log(tick_data[ str(table.stock2[pair]) ])
    #print(spread)
    ...
    for i in range( len(spread)-2 ):
        ...
        if position == 0 and len(spread)-i > M and i <= 40 :      
                                         # 之前無開倉
            #print(spread[i],spread[i+1])
            if ( spread[i] - up_open ) * ( spread[i+1] - up_open ) < 0 :
                    
                # 資金權重轉股票張數，並整數化
                w1 , w2 = num_weight( table.w1[pair] , table.w2[pair] , tick_data[str(table.stock1[pair])][(i+1)] , tick_data[str(table.stock2[pair])][(i+1)] , maxi , capital )          
                ...
                if adfuller( spread1 , regression='c' )[1] > 1:                                   # spread平穩才開倉
                    ...
                    #print("配對價格 :",stock1_payoff,stock2_payoff)
            ...
            num = fore_chow( min_price[str(table.stock1[pair])].loc[0:t] , min_price[str(table.stock2[pair])].loc[0:t] , stock1_seq , stock2_seq )
            ...
            elif ( spread[i] - close ) * ( spread[i+1] - close ) < 0  :
    ...
    #print("trade :",trade)
    #print( "local profit", trading_profit)
    ...
    if trade == 0:            # 如果都沒有開倉，則報酬為0
        ...
        spread2 = w1 * np.log(min_price[str(table.stock1[pair])].iloc[0:t]) + w2 * np.log(min_price[str(table.stock2[pair])].iloc[0:t])
