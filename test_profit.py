...
def reward(action_list):
    ...
    profit_count = 0
    for date in sorted(datelist[197:]): #決定交易要從何時開始
        ...
        for pair in num: #看table有幾列配對 依序讀入
            ...
            profit, opennum,trade_capital,_  = trading_period_by_gate_nocheck.pairs( pair ,166,  table , mindata , tickdata , open ,open, loss ,mindata, max_posion , 0, 0 , 300000000 )
            """
            if profit > 0 and opennum == 1 :
                profit_count +=1
                print("有賺錢的pair",profit)
                
                flag = os.path.isfile('C:/Users/Allen/pair_trading DL2/3_ResNet_2018_profit pairs201706/'+str(date)+'_profit.csv')
